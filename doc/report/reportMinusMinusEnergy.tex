...
\begin{lstlisting}[language=Solidity, firstnumber=28]
...
\begin{lstlisting}[language=Solidity, firstnumber=68]
...
\end{lstlisting}
As one can see, the distribution of the money from the fund is not yet implemented. This is not done already, because we asked ourselves, how the gas is payed (which is needed for the transactions). We couldn't decide wether we just subtract the amount needed from the fund or if we just assume that a third party will pay for the cost.
