...
\end{wraptable}
A swiss household in a one-family house with 4 persons consumes at average 5200 kWh per year\footnote{\href{https://www.bundespublikationen.admin.ch/cshop_mimes_bbl/2C/2C59E545D7371ED5BB89418480F1B62D.pdf}{Bundespublikationen 805.902.D}} paying around 1372.- CHF.\footnote{\href{https://www.ewz.ch/content/dam/ewz/services/dokumentencenter/energie-beziehen/dokumente/gruener-strom-fuer-mein-zuhause/stromtarif-2018-zh-private.pdf}{EWZ Zürich}: 26.39 Rp./kWh} The government defines a target reduction of 10\% of the average energy consumption equal to a reduction of 84.8 kg CO\textsubscript{2}\footnote{\href{https://www.bafu.admin.ch/bafu/de/home/suche.html\#R071-1222}{BAFU R071-1222}: 169 g CO\textsubscript{2}/kWh} per year and subsidies at maximum of 120.- CHF per household. \\\\
An end consumer uses 4940 kWh in one year equal to 5\% below the average consumption. This values are generated by a smart meter installed by the energy provider. The data is regularly send to a client server to analyse and visualize the energy consumption for the end consumer. On a monthly basis the sensor data is used to claim token for the reduced energy consumption.
...
\begin{figure}[ht]
	\begin{center}
		\includegraphics[scale=1.7]{concept.png}
...
\noindent
At the end of the year the energy provider charges the end consumer with 939.- CHF for his usage. The end consumer decides to split his tokens on a 40/60 ratio between his own invoice and the fond. Therefore he has to pay the energy provider
\begin{equation}
1304-0.4*57=1281  \textrm{ CHF}
\end{equation}
The amount of money the subsidizing party is paying to the energy provider is equal to
\begin{equation}
0.4*57=23 \textrm{ CHF}
\end{equation}
and the money the subsidizing party is transferring to the fond with a reference of the end consumer (for the voting rights and shares) amounts to
\begin{equation}
0.6*57=34 \textrm{ CHF}
\end{equation}
...
During the BETH-Event, we created individual modules, each will be discussed later. The architecture consist of a sensor, the client server and three smart contract on the ethereum blockchain. A small overview is presented in figure \ref{img_impl} on page \pageref{img_impl}.\\\\
\begin{figure}[ht]
	\begin{center}
...
The sensor data is cryptographically secured using elliptic curve cryptography signature (ECC) \cite{24} inside the sensor and then send to the MME contract. The contract validates the signature of the data and also prevents replay attacks.\footnote{A replay attack is a form of network attack in which a valid data transmission is maliciously or fraudulently repeated.}
\paragraph{Bill Contract}
The bill contract handles the creation of invoices for clients by the energy providers and the payment of those. The currency of the invoices is ether and all transaction dealt on the ethereum platform. The owner of the contract is able to add new invoice issuer. When a payment is made to the bill contract the client includes the desired ratio of available tokens to use for a reduction of his invoice.
\paragraph{Fund Contract}
The fond contract stores the amount of money each client has invested. In the future the fond contract can be extended with a voting system to reinvest the saved assets. At the current state only the owner of the fond is able to release the assets.
